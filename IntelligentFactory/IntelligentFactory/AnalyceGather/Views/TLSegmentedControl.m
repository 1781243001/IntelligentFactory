...
-(instancetype)initWithFrame:(CGRect)frame titls:(NSArray *)titles  hideAdd:(BOOL)isShow delegate:(id<TLSegmentedControlDelegate>)delegate {
...
-(void)addAction:(UIButton *)sender{
   ...
   JLSegmentedButton *btn =(JLSegmentedButton *) sender.superview;
    NSInteger index = [self.btns indexOfObject:btn];
    if (lastIndex == index) {
        DetailedInformationModel *model = [_segmentedTitles objectAtIndex:sender.tag];
...
-(void)setSegmentedTitles:(NSArray *)segmentedTitles {
    ...
    NSInteger i = 0;
    for (DetailedInformationModel *model in self.segmentedTitles) {
        JLSegmentedButton *segBtn = [JLSegmentedButton buttonWithType:UIButtonTypeCustom];
        [segBtn setTitle:model.text forState:UIControlStateNormal];
        ...
        [segBtn sizeToFit];
        if (_isshowAdd) {
            CGFloat with = segBtn.titleLabel.intrinsicContentSize.width+50;
            ...
            segBtn.bounds = CGRectMake(0, 0, segBtn.titleLabel.intrinsicContentSize.width,self.frame.size.height);
        }
...
-(void)layoutSubviews {
    ...
    for (JLSegmentedButton *btn in self.btns) {
        if (_isshowAdd) {
            btnsWidth += btn.intrinsicContentSize.width+50;
        }else{
    ...
    self.indicatorBar.center = CGPointMake(CGRectGetMidX(firstBtn.frame), self.height-3);
}
@end
