$(function(){
	...
	var jlZhzAll = baseObj.getQuery(window.location.search, 'sjson');
	//var DepartmentAll = '[{"id":"5466","text":"化工部东区","orderId":0,"selected":1},{"id":"5461","text":"化工部西区","orderId":0,"selected":1}]'
	/*var P = '<p>'+jlZhzAll+'</p><p>第三方斯蒂芬</p>';
	$("body").append(P)*/
	
	if(jlZhzAll == ""){
		//bm_mrAreaListData()
	} else {
		localStorage.setItem("jlZhz",jlZhzAll);
		var jlZhz = JSON.parse(localStorage.getItem("jlZhz"));
		console.log(jlZhz)
		//bm_dzAreaListData(jlZhz)
	}
