...
$(function(){
	...
	function bm_dzAreaListData(Department){
		$(".chdzListBody").empty();
    	...
    	$.each(Department, function(i) {
    		listDome += '<section class="outBox" style="margin-top: 0px;">'+
							'<div class="title clear">'+
								'<div data-areaId='+Department[i].id+' class="left-c areaId">'+decodeURIComponent(decodeURIComponent(Department[i].text))+'</div>'+
							'</div>'+
							'<div id="ch_index_'+Department[i].id+'" class="chfx_cbox_dz"></div>'+
						'</section>';
    	});
    	...
    	$(".seeCh_Detail").on("click",function(){
    		...
    		if(areaId == 2901){
    			window.location.href = "chanHaoFXFactory.html?areaId="+areaId+"&areaName="+areaName+"&FactoryData=";
    		} else {
    			window.location.href = "chanHaoFXWorkshop.html?areaId="+areaId+"&areaName="+areaName+"&FactoryData=";
    		}
    	})
	...
	function bm_mrAreaListData(){
		...
		$.ajax({
	        ...
	        success: function(msg){
        		console.log(JSON.parse(msg))
	        	var listData = JSON.parse(msg).treeList[0].children;
	        	console.log(listData)
	        	var listDome = '<section class="outBox" style="margin-top: 0px;">'+
									'<div class="title clear">'+
										'<div class="left-c">'+JSON.parse(msg).treeList[0].text+'</div>'+
									'</div>'+
									'<div id="ch_index_'+JSON.parse(msg).treeList[0].id+'" class="chfx_cbox_mr"></div>'+
								'</section>';
	        	var listName = ""
	        	$.each(listData, function(i) {
	        		if(listData[i].id == 2901){
						listName = '<div data-areaId='+listData[i].id+' class="seeCh_Detail">厂区列表</div>';
					}
	        		if(listData[i].id != 2901){
						listName = '<div data-areaId='+listData[i].id+' class="seeCh_Detail">车间列表</div>';
					}
	        		listDome += '<section class="outBox" style="margin-top: 0px;">'+
									'<div class="title clear">'+
